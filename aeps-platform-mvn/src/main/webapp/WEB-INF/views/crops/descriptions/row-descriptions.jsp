        ...
        <% if (entTypeDesId!=3) { %>
            <div class="btn-group">
                ...
                <% if (usrDesDao.getPrivilegeUser(userDes.getIdUsr(), "crop/delete")) { %>
                    <a class="btn btn-small delete_rows_dt btn-delete" title="<s:property value="getText('link.obsdelete.observation')" />" onclick="showDialogDelete(this, 'confirm_dialog_des', '/crop/deleteDescrip.action?idDesPro=${idDesPro}', '/crop/searchDescrip.action?idCrop=${idCrop}', 'divDes', 'divListDes'); $('.confirm_yes').click(function(){setTimeout(function() {showTimeline('/crop/viewInfoTime.action?idCrop=${idCrop}', 'divInfoTimeline', 'timeline')}, 2000);});"><i class="icon-trash"></i></a>
                <% } %>
            </div>
        <% } %>
    <% } %>
