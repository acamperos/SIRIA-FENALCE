        ...
        <% if (entTypePrpId!=3) { %>
            <div class="btn-group">
                ...
                <% if (usrPrpDao.getPrivilegeUser(userPrp.getIdUsr(), "crop/delete")) { %>
                    <a class="btn btn-small btn-default delete_rows_dt btn-delete" title="<s:property value="getText('link.deletepreparation.preparation')" />" onclick="showDialogDelete(this, 'confirm_dialog_prep', '/crop/deletePrep.action?idPrep=<s:property value="idPrep" />', '/crop/searchPrep.action?idCrop=${idCrop}', 'divPrep', 'divListPrep'); $('.confirm_yes').click(function(){setTimeout(function() {showTimeline('/crop/viewInfoTime.action?idCrop=${idCrop}', 'divInfoTimeline', 'timeline')}, 2000);});"><i class="icon-trash"></i></a>
                <% } %>
            </div>
        <% } %>
    <% } %>
